...
散射适配器内部沿用 libphysica 自然单位，`SolarBackground` 公共查询使用显式 cgs；跨语言文件使用 `r_cm`、`v_cm_s`、`rate_s_inv`、`source_particles_s`、`mass_GeV`、`kBT_eV`。势和比能量采用 `cm²/s²`。旧代码的 `E_capture_eV` 是粒子能量，不可直接当作比能量 `v²/2+Phi`。
