...
项目处于初步开发阶段，已具备有限体积网格、基准配置和测试入口；完整输运求解器及物理验证尚未完成。
...
参数见 [MVP 配置](Code/configs/benchmark/mvp.json)。[基线工具](Code/python/run_baseline.py) 依赖另行配置的 DaMaSCUS-SUN-EVAP，使用 `--help` 查看参数。当前测试覆盖网格和项目约定，尚不代表输运物理验收通过。
