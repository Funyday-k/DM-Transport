...
项目处于初步开发阶段。T03 局域碰撞物理门已通过；T04 已完成有限体积网格、阈值穿越子格测度、保守点源投影和 C++/Python 网格 schema。T05 已有最小参考碰撞生成元行，完整碰撞核与输运求解器尚未完成。固定 MVP 为 `m_chi=0.1 GeV`、SD constant-contact、proton-only，太阳背景采用冻结 AGSS09 数据。
